...
def train(length,folds, num_epochs, batch_size, n):
	...
	if warmstart:
		...
		optimizer = optim.Adam(paramList[0:(paraml-2)],lr=0.1*lr,betas=(beta1,beta2))
		optimize2 = optim.Adam(paramList[(paraml-2):(paraml-1)],lr=20*lr,betas=(beta1,beta2))	#slope
		optimize3 = optim.Adam(paramList[(paraml-1):paraml],lr=10*lr,betas=(beta1,beta2))	#bias
