...
class Fit:
    ...
    def removeBKG(x,y,bkg,**kwargs):
        try:
            indx = np.array([i for i in range(len(x)) if bkg[i]!=0]).astype(int)
            xs=x[indx]
            bs=bkg[indx]
    ...
    def __energyResolution(x,**kwargs):
        ...
        bin_centers = np.mean(np.vstack([binedges[:-1], binedges[1:]]), axis=0)
        ...
        bin_centers2 = np.mean(np.vstack([binedges2[:-1], binedges2[1:]]), axis=0)
