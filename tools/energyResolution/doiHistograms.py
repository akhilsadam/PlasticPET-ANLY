...
def pltDOI(counts,gammaZ,axs,cmap,legend,x,y):
    ...
    for i in range(1,nplt):
        indx = np.intersect1d(np.where(gammaZ < zval2[i]), np.where(gammaZ > zval2[i-1]), assume_unique=True, return_indices=False)
        # print(indx)
        # print(zval2[i])
        counting = counts[indx]
        # print(counting)
        axs.hist(counting,bins = int(mx/histogramOptions.binwidth_1),range = [0,mx], color = cmap[i-1],alpha = 0.3,stacked = False)
